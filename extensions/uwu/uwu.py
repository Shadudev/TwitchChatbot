...
class UwUHandler(CommandHandler):
	...
	def handle_message(self, chat_message):
		if self.is_user_on_cooldown(chat_message.user):
			...
			remaining_cooldown = self._cooldown_manager.get_remaining_cooldown(COMMAND_ID, chat_message.user, user_cooldown).seconds
			self.send_message("{}'s !uwu is on cooldown for {} more seconds".format(chat_message.user, remaining_cooldown))
			return
