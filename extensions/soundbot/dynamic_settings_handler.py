...
class DynamicSettingsHandler(CommandHandler):
    ...
    def is_get_command(self, command_args):
        if len(command_args) < 2:
            return False
        return self._settings.does_sound_exist(command_args[SOUND_ID_INDEX]) and self._settings.does_property_exist(
            command_args[PROPERTY_INDEX].lower())
    ...
    def is_mod_command(self, command_args):
        if self.is_get_command(command_args):
            return True
        if len(command_args) > MOD_OPERATION_INDEX and command_args[
            MOD_OPERATION_INDEX].lower() in DynamicSettingsHandler.MOD_SOUND_OPERATIONS:
            return True
    ...
    def handle_get_command(self, command_args):
        ...
        if property_key == ALL_MAGIC:
            ...
            self.send_message(
                '{}.{}={}'.format(sound_id, property_key, self._settings.get_property(sound_id, property_key)))
